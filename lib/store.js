...
// En Vercel el disco es de solo-lectura salvo /tmp. Sin Supabase, usamos /tmp con datos demo.
const ON_VERCEL = !!process.env.VERCEL;
const FILE = ON_VERCEL ? '/tmp/avanzzaos_store.json' : path.join(__dirname, '..', 'data', 'store.json');

function backendName() { return useSupabase ? 'supabase' : (ON_VERCEL ? 'file(/tmp demo)' : 'file'); }
...
function fileGet() {
  try {
    ...
    // En Vercel sin datos aún: arrancar desde el dataset demo.
    if (ON_VERCEL) { try { return { data: require('./seed'), meta: {} }; } catch (e2) {} }
    return { data: DEFAULT_STATE, meta: {} };
  }
}
